...
可以写 HTML/CSS/JS。写出来的样式必须服务已选方向，不能当默认皮肤。字体名不是授权；只用项目已有、系统栈或已核许可的字体。

受约束的生成器（例如只许改槽位、不许吐 CSS）先读项目叠加层，不要硬写样式。
...
2. 用一句话说出样子，且这句话换到另一类产品会不合适。例如「车间规格表，氧化红印在新闻纸上」，不是「现代、干净、高级」。
